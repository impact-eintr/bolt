...
func (n *node) write(p *page) {
	...
	for i, item := range n.inodes {
		...
		if n.isLeaf {
			...
			//   ^                b[0] (b[0]的初始位置 注意b是一个slice)
			//   这样对于b1而言 他到k1的距离就是
			//   &b[0] - &b1 == uintptr(unsafe.Pointer(&b[0])) - uintptr(unsafe.Pointer(elem))

			// {[b1 b2 b3 b4 ...][k1 k2 k3 k4 ...]}
			//      ^                b[0] (b[0]的下一个位置)
			//   这样对于 b2 而言 他到 k2 的距离就是
			//   &b[0] - &b2 == uintptr(unsafe.Pointer(&b[0])) - uintptr(unsafe.Pointer(elem))
...
func (n *node) put(oldKey, newKey, value []byte, pgid pgid, flags uint32) {
	if pgid >= n.bucket.tx.meta.pgid {
		...
		panic("put: zero-length olg key")
	} else if len(newKey) <= 0 {
		panic("put: zero-length new key")
	}

	// TODO oldKey 和 newKey
	// 二分查找到待插入位置
...
// 分裂
// 当一个node中的数据过多时，最简单就是当超过了page的填充度时，
...
func (n *node) spill() error {
	...
	// as well. We'll
	// clear out the children to make sure it doesn't try to respill.
...
func (n *node) splitTwo(pageSize int) (*node, *node) {
	// 太小的话就不拆分了
	if len(n.inodes) <= (minKeyPerPage*2) || n.sizeLessThan(pageSize) {
		return n, nil
	}
...
func (n *node) splitIndex(threshold int) (index, sz int) {
	...
	for i := 0; i < len(n.inodes)-minKeyPerPage; i++ {
		index = i
		inode := n.inodes[i]
		elsize := n.pageElementSize() + len(inode.key) + len(inode.value)

		if i >= minKeyPerPage && sz+elsize > threshold {
			break
		}
		sz += elsize
	}
